...
\begin{figure}[ht]
  \centering
  \input{ch07/figures/fig-darboux-upper-lower.tex}
...
\begin{figure}[ht]
  \centering
  \resizebox{\textwidth}{!}{\input{ch07/figures/fig-lower-upper-sums-numberline.tex}}
...
\begin{figure}[ht]
  \centering
  \input{ch07/figures/fig-single-point-spike.tex}
...
\begin{figure}[ht]
  \centering
  \input{ch07/figures/fig-dirichlet-function.tex}
...
\begin{figure}[ht]
  \centering
  \resizebox{0.85\textwidth}{!}{\input{ch07/figures/fig-nonintegrable-dense-oscillation.tex}}
...
\begin{figure}[ht]
  \centering
  \input{ch07/figures/fig-riemann-rectangles.tex}
...
\label{thm:7-11}
Let $f$ be bounded and integrable on $[a,b]$.  Let $(P_n)$ be a sequence of partitions of $[a,b]$, each having $n$ sub-intervals, with $\norm{P_n} \to 0$, and for each $P_n$, choose sample points $x_i^{*}$ on each sub-interval.  Then
\[
...
\begin{remark}
This also makes sense geometrically: when $f$ and $g$ are non-negative, the region under the graph of $f$ is contained in the region under the graph of~$g$, so its area is no larger.
\end{remark}
