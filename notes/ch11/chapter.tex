...
\begin{enumerate}
  ...
  \[
    F_{0}=1,\quad F_{1}=1,\quad F_{n}=F_{n-1}+F_{n-2}\;\text{for }n\ge 2.
  \]
\end{enumerate}
...
\begin{figure}[ht]
  \centering
  \input{ch11/figures/fig-sequence-epsilon-neighborhood.tex}
...
\begin{figure}[ht]
  \centering
  \input{ch11/figures/fig-monotone-bounded-convergence.tex}
...
\textit{We now prove \cref{thm:11-6} in the case of an increasing sequence that is bounded above.  The proof for a decreasing, bounded-below sequence is entirely analogous.  The key ingredient is the least-upper-bound axiom for $\R$.}
...
\begin{proof}
Let $\{a_{n}\}$ be an increasing sequence that is bounded above.
...
Fix an arbitrary $n\ge n_{0}$.  Because the sequence is increasing and $n\ge n_{0}$,
\[
  a_{n_{0}}\le a_{n}.
\]
...
\begin{figure}[ht]
  \centering
  \input{ch11/figures/fig-growth-hierarchy.tex}
...
Fix $p>0$ and $c>1$.  Define $f(x)=\frac{x^{p}}{c^{x}}$ for $x\ge 1$.  Since both numerator and denominator tend to $\infty$, we apply L'H\^opital's Rule repeatedly.  After $k=\lceil p\rceil$ applications, the numerator becomes a constant while the denominator still tends to $\infty$, so the limit is $0$.  By \cref{thm:11-3}, $\lim_{n\to\infty}\frac{n^{p}}{c^{n}}=0$.
