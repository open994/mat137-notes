...
\begin{figure}[ht]
  \centering
  \input{ch08/figures/fig-accumulation-function.tex}
...
\begin{figure}[ht]
  \centering
  \input{ch08/figures/fig-net-change-signed-area.tex}
...
\begin{figure}[ht]
  \centering
  \input{ch08/figures/fig-ftc-commutative-diagram.tex}
...
\begin{itemize}
  ...
  \]
  \item \textbf{Indefinite integral:} The result is a collection of functions \textit{of the named variable}.  If we change all $x$'s to $t$'s, we get a collection of functions of $t$, which is not the same expression:
  \[
    \int f(x)\dx \ne \int f(t)\dt \quad \text{(different variable names)}.
  \]
