...
\begin{figure}[ht]
  \centering
  \input{ch13/figures/fig-partial-sums-converge.tex}
...
\begin{figure}[ht]
  \centering
  \input{ch13/figures/fig-geometric-series-area.tex}
...
\label{def:13-2}
For the purpose of convergence, only what happens for large values of $n$ matters.  We call the portion of a series for large $n$ the \emph{tail} of the series.
\end{definition}
...
\begin{figure}[ht]
  \centering
  \input{ch13/figures/fig-divergence-test.tex}
...
\begin{figure}[ht]
  \centering
  \input{ch13/figures/fig-alternating-series-estimation.tex}
...
\[
  S \;\approx\; S_{5} = \sum_{n=1}^{5}\frac{(-1)^{n}}{n^{4}} = -1+\frac{1}{16}-\frac{1}{81}+\frac{1}{256}-\frac{1}{625} \approx -0.94753.
\]
...
\textit{Given a series $\sum a_{n}$, write $\mathrm{P.T.}$ for the sum of the positive terms (replacing negative terms by~$0$) and $\mathrm{N.T.}$ for the sum of the negative terms (replacing positive terms by~$0$).  For finite sums, $\sum a_{n}=\mathrm{P.T.}+\mathrm{N.T.}$ is obvious.  For infinite sums, this splitting is valid \textbf{only when both $\mathrm{P.T.}$ and $\mathrm{N.T.}$ converge}.}
...
\begin{enumerate}[label=(\roman*)]
  \item $\lim_{n\to\infty}a_{n}=0$ (true for any convergent series, by the Divergence Test).
